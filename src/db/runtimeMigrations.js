...
async function ensureAiTables() {
  await Promise.all([
    ...
    bestEffort(`CREATE TABLE IF NOT EXISTS ai_image_usage (user_id INTEGER NOT NULL REFERENCES users(id) ON DELETE CASCADE, usage_month DATE NOT NULL, images_used INTEGER NOT NULL DEFAULT 0 CHECK (images_used >= 0), PRIMARY KEY (user_id, usage_month))`),
...
async function ensureBillingTables() {
  ...
  await bestEffort('ALTER TABLE billing_plan_changes ADD COLUMN IF NOT EXISTS meu_ecoo_email_status TEXT')
  await bestEffort('ALTER TABLE billing_plan_changes ADD COLUMN IF NOT EXISTS meu_ecoo_email_attempts INTEGER NOT NULL DEFAULT 0')
...
async function runMigrations() {
  await Promise.all([
    ...
    bestEffort("ALTER TABLE posts ADD COLUMN IF NOT EXISTS facebook_format TEXT DEFAULT 'post'"),
    ...
    bestEffort('ALTER TABLE posts ADD COLUMN IF NOT EXISTS media_cleanup_after TIMESTAMPTZ'),
    bestEffort('ALTER TABLE posts ADD COLUMN IF NOT EXISTS media_cleaned_at TIMESTAMPTZ'),
    bestEffort('CREATE INDEX IF NOT EXISTS idx_posts_media_cleanup ON posts (media_cleanup_after) WHERE media_cleanup_after IS NOT NULL AND media_cleaned_at IS NULL'),
    ...
    `).then(() => Promise.all([
      ...
      bestEffort("ALTER TABLE drafts ADD COLUMN IF NOT EXISTS facebook_format TEXT DEFAULT 'post'"),

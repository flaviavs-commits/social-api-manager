...
router.post('/', async (req, res) => { try { if (!req.body?.name?.trim()) return res.status(400).json({ erro: 'Informe um nome para a API key.' }); const raw = `mk_${crypto.randomBytes(32).toString('hex')}`; const hash = crypto.createHash('sha256').update(raw).digest('hex'); const prefix = raw.slice(0, 12); const { rows } = await pool.query('INSERT INTO api_keys (user_id,name,prefix,key_hash) VALUES ($1,$2,$3,$4) RETURNING id', [req.user.id, req.body.name.trim(), prefix, hash]); res.status(201).json({ id: rows[0].id, key: raw, prefix }) } catch (err) { serverError(res, err) } })

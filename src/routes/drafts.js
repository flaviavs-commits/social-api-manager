  ...
  try {
    const { rows } = await pool.query(
      `SELECT id, title, text, platforms, media_path, media_type, media_items,
              ...
              location_id, location_name, first_comment, threads_reply_control
...
router.post('/', async (req, res) => {
  try {
    const {
      ...
      locationId, locationName, firstComment, threadsReplyControl
    } = req.body
    const { rows } = await pool.query(
      `INSERT INTO drafts (
      ...
      [req.user.id, title||null, text||null, platforms||[], mediaPath||null, mediaType||null,
       ...
       locationId||null, locationName||null, firstComment||null, threadsReplyControl||null]
    )

  ...
  try {
    ...
    const { rows } = await pool.query(
      `SELECT id, title, text, platforms, media_path, media_type, media_items,
              ...
              ig_format, facebook_format, youtube_format, youtube_category_id, youtube_made_for_kids,
...
router.post('/', async (req, res) => {
  try {
    const {
      ...
      textByPlatform, mediaByPlatform, igFormat, facebookFormat, youtubeFormat, youtubeCategoryId, youtubeMadeForKids,
    ...
    const { rows } = await pool.query(
      `INSERT INTO drafts (
         ...
         text_by_platform,media_by_platform,ig_format,facebook_format,youtube_format,youtube_category_id,youtube_made_for_kids,
         tiktok_privacy_level,tiktok_disable_comment,tiktok_disable_duet,tiktok_disable_stitch,
         location_id,location_name,first_comment
       )
      ...
      [req.user.id, title||null, text||null, platforms||[], mediaPath||null, mediaType||null,
       ...
       igFormat||null, facebookFormat||null, youtubeFormat||null, youtubeCategoryId||null, youtubeMadeForKids||null,
...
router.patch('/:id', async (req, res) => {
  try {
    ...
    const { text, textByPlatform, platforms, igFormat, facebookFormat, youtubeFormat } = req.body || {}
    const { rowCount } = await pool.query(
      `UPDATE drafts SET text=$1, text_by_platform=$2, platforms=$3, ig_format=$4, facebook_format=$5, youtube_format=$6 WHERE id=$7 AND user_id=$8`,
      [typeof text === 'string' ? text : null, textByPlatform && Object.keys(textByPlatform).length ? JSON.stringify(textByPlatform) : null, Array.isArray(platforms) ? platforms : [], igFormat || null, facebookFormat || null, youtubeFormat || null, id, req.user.id]
    )

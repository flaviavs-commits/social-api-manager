  ...
  try {
    const { rows } = await pool.query(
      `SELECT id, title, text, platforms, media_path, media_type, media_items,
              ...
              location_id, location_name, first_comment, threads_reply_control, linkedin_visibility,
...
router.post('/', async (req, res) => {
  try {
    const {
      ...
      locationId, locationName, firstComment, threadsReplyControl, linkedinVisibility,
    ...
    const { rows } = await pool.query(
      ...
      [req.user.id, title||null, text||null, platforms||[], mediaPath||null, mediaType||null,
       ...
       locationId||null, locationName||null, firstComment||null, threadsReplyControl||null, linkedinVisibility||null,
       pinterestBoardId||null, pinterestBoardName||null]
    )

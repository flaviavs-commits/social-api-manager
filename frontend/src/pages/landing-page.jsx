...
export function LandingPage() {
  ...
  return <main className="landing-page" id="inicio">
    ...
    <section className="landing-section landing-plans landing-container" id="planos"><div className="landing-plans-heading"><div><div className="landing-section-kicker">PLANOS PARA CADA MOMENTO</div><h2>Escolha o plano ideal<br />para suas redes.</h2></div><p>Comece com o essencial e evolua quando sua presença digital crescer.</p></div><div className="landing-plans-grid">{plans.map(plan => <article className={`landing-plan${plan.featured ? ' is-popular' : ''}`} key={plan.id}>{plan.featured && <div className="landing-popular-badge">Mais escolhido</div>}<div className="landing-plan-top"><div><span>TIER {plan.tierNumber}</span>{plan.offer && <small className="landing-plan-offer">{plan.offer}</small>}{plan.meuEcooAccess !== 'none' && <a className="landing-plan-meuecoo-link" href="http://www.meuecoo.com/" target="_blank" rel="noopener noreferrer">Conheça o MeuEcoo</a>}<h3>{plan.name.replace('EcooMidia ', 'EcooMidia\n')}</h3></div><div className="landing-plan-price"><strong>{plan.price}</strong><small>{plan.cadence}</small></div></div><p>{plan.description}</p><ul>{plan.features.map(feature => <li key={feature}>{feature}</li>)}</ul>{plan.meuEcooFeatures?.length > 0 && <div className="landing-plan-meuecoo-box"><div className="landing-plan-meuecoo-heading"><Icon name="sparkle" size={15} /><strong>MeuEcoo incluído</strong></div><ul>{plan.meuEcooFeatures.map(feature => <li key={feature}>{feature}</li>)}</ul></div>}<a className={`landing-plan-btn${plan.featured ? ' is-primary' : ''}`} href={`/criar-conta?plan=${plan.id}`}>Assinar {plan.name} <Icon name="arrow" size={15} /></a></article>)}</div></section>

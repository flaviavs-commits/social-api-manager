        ...
        {!selected ? <section className="workspace-empty-main"><div className="workspace-empty-main-icon" aria-hidden="true">✦</div><h3>Seu primeiro espaço começa aqui</h3><p>Crie um espaço ao lado para centralizar pessoas, aprovações e identidade visual de uma marca.</p></section> : <>
          ...
          <div className="workspace-content-grid">
            ...
            <section className="workspace-card workspace-approval-card">
              ...
              <form className="workspace-approval-form" onSubmit={requestApproval}><label htmlFor="approval-post">Escolha uma publicação</label><div className="workspace-member-fields"><select id="approval-post" value={form.postId} onChange={event => setForm(current => ({ ...current, postId: event.target.value }))} required><option value="">Selecione um post...</option>{posts.slice(0, 30).map(post => <option key={post.id} value={post.id}>#{post.id} · {(post.text || 'Publicação').slice(0, 54)}</option>)}</select><button className="action-button" type="submit">Enviar para revisão</button></div></form>

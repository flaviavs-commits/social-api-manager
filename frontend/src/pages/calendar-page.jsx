    ...
    <section className="mx-auto max-w-6xl px-6 py-8">
      <div className="mb-6 flex items-center justify-between">
        <div>
          ...
          <h2 className="text-2xl font-semibold text-zinc-50">{String(month).padStart(2, '0')}/{year}</h2>
        </div>
      ...
      {error && <p className="mb-4 rounded-lg border border-red-500/30 bg-red-500/10 px-4 py-2 text-sm text-red-400">{error}</p>}

      <div className="grid grid-cols-7 overflow-hidden rounded-xl border border-subtle bg-surface">
        {['Dom', 'Seg', 'Ter', 'Qua', 'Qui', 'Sex', 'Sáb'].map(label => (
          <div key={label} className="border-b border-subtle px-2 py-2 text-center text-[11px] font-semibold uppercase tracking-wide text-zinc-500">
            {label}
          </div>
        ))}

        {Array.from({ length: firstWeekday }, (_, index) => (
          <div key={`empty-${index}`} className="min-h-[110px] border-b border-r border-subtle bg-app/40" />
        ))}
        ...
        {Array.from({ length: days }, (_, index) => {
          ...
          return (
            <div key={day} className="min-h-[110px] border-b border-r border-subtle p-2 last:border-r-0">
                ...
                {dayPosts.map(post => (
                  ...
                  >
                    <div className="flex items-center gap-1.5">
                      {platformsOf(post).map(platform => <PlatformIcon key={platform} platform={platform} />)}
                      <span className="truncate">{post.text || post.title || 'Publicação'}</span>
                    </div>

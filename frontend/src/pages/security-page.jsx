...
export function SecurityPage({ user }) {
  ...
  return <section className="page-view security-page"><section className="panel security-panel">
    ...
    {!enabled && !setup && <div className="security-intro"><div className="security-icon" aria-hidden="true">⌁</div><div><h3>Ative a autenticação em 2 fatores</h3><p>Depois de ativar, além da senha você precisará de um código temporário do Google Authenticator, Authy ou outro app compatível.</p><label>Senha atual<input type="password" value={password} onChange={event => setPassword(event.target.value)} autoComplete="current-password" /></label><button type="button" className="action-button" onClick={startSetup} disabled={busy}>{busy ? 'Gerando…' : 'Configurar 2FA'}</button></div></div>}

...
export function getMediaLimits(platform, { mediaKind = 'image', format, youtubeFormat } = {}) {
...
export function validateMediaMetadata({ platform, mediaKind, format, youtubeFormat, size, duration, width, height }) {
  ...
  if (mediaKind === 'video' && isKnownNumber(width) && isKnownNumber(height) && limits.minWidth && (width < limits.minWidth || height < limits.minHeight))
  ...
  if (mediaKind === 'video' && isKnownNumber(duration)) {
    ...
    if (limits.maxDurationSeconds !== undefined) {
      const acimaDoLimite = limits.maxDurationExclusive ? duration >= limits.maxDurationSeconds : duration > limits.maxDurationSeconds
      if (acimaDoLimite)
        violations.push({ code: limits.maxDurationExclusive ? 'max-duration-exclusive' : 'max-duration', value: duration, limit: limits.maxDurationSeconds })
    }
  }
...
export function formatMediaLimitViolation(platform, { mediaKind, format, youtubeFormat }, violation) {
  ...
  if (violation.code === 'min-resolution') return 'O vídeo do TikTok precisa ter no mínimo 720p (720 × 1280 px).'
  if (violation.code === 'min-duration') return `O vídeo do TikTok precisa ter pelo menos ${formatSeconds(violation.limit)}.`
  if (violation.code === 'max-duration-exclusive') return `${subject} precisa ter menos de ${formatSeconds(violation.limit)}.`
  if (violation.code === 'max-duration') return `${subject} pode ter no máximo ${formatSeconds(violation.limit)}.`
